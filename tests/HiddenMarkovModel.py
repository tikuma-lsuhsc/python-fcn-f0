...
class HiddenMarkovModel(object):
    ...
    def initialize_viterbi_variables(self, shape):
        ...
        states_seq = tf.Variable(
            tf.zeros([shape[0]], dtype=tf.int64), name="States_sequence"
        )
    ...
    def initialize_forw_back_variables(self, shape):
        ...
        self.posterior = tf.Variable(
            tf.zeros(shape, dtype=tf.float64), name="posteriror"
        )
    ...
    def _forward(self, obs_prob_list):
        ...
        with tf.name_scope("forward_first_step"):
            ...
            self.scale = tf.compat.v1.scatter_update(
                self.scale, 0, 1.0 / tf.reduce_sum(init_prob)
            )

            # scaled belief at t=0
            self.forward = tf.compat.v1.scatter_update(
        ...
        for step, obs_prob in enumerate(obs_prob_list[1:]):
            with tf.name_scope("time_step-%s" % step):
                ...
                # scaling factor
                self.scale = tf.compat.v1.scatter_update(
                    self.scale, step + 1, 1.0 / tf.reduce_sum(forward_prob)
                )
    ...
    def _backward(self, obs_prob_list):
        with tf.name_scope("backward_last_step"):
            ...
            self.backward = tf.compat.v1.scatter_update(
                self.backward,
                0,
                self.scale[self.N - 1] * tf.ones([self.S], dtype=tf.float64),
            )
        ...
        for step, obs_prob in enumerate(obs_prob_list[:-1]):
            with tf.name_scope("time_step-%s" % step):
                ...
                self.backward = tf.compat.v1.scatter_update(
                    self.backward,
                    step + 1,
                    self.scale[self.N - 2 - step] * backward_prob,
                )

        self.backward = tf.compat.v1.assign(
    ...
    def re_estimate_emission(self, x):
        ...
        emission_score = tf.matmul(
            tf.cast(seq_one_hot, tf.float64), self.gamma, transpose_a=True
        )
    ...
    def re_estimate_transition(self, x):
        ...
        with tf.name_scope("3D_tensor_transition"):
            for t in range(self.N - 1):
                with tf.name_scope("time_step-%s" % t):
                    ...
                    tmp_1 = tf.multiply(
                        tmp_0, tf.expand_dims(tf.gather(self.E, x[t + 1]), 0)
                    )
                    denom = tf.squeeze(
                        tf.matmul(tmp_1, tf.expand_dims(self.backward[t + 1, :], 1))
                    )

                with tf.name_scope("Init_new_transition"):
                    trans_re_estimate = tf.Variable(
                        tf.zeros((self.S, self.S), tf.float64)
                    )
        ...
        with tf.name_scope("Smooth_gamma"):
            ...
            T_new = tf.reduce_sum(self.M, 0) / tf.expand_dims(
                tf.reduce_sum(self.gamma, 0), 1
            )
        ...
        with tf.name_scope("Append_gamma_final_time_step"):
            prod = tf.expand_dims(
                tf.multiply(self.forward[self.N - 1, :], self.backward[self.N - 1, :]),
                0,
            )
    ...
    def check_convergence(self, new_T0, new_transition, new_emission):
        ...
        return tf.math.logical_and(tf.math.logical_and(delta_T0, delta_T), delta_E)
    ...
    def expectation_maximization_step(self, x):
        ...
        with tf.name_scope("Update_parameters"):
            self.T0 = tf.compat.v1.assign(self.T0, new_T0)
            self.E = tf.compat.v1.assign(self.E, new_emission)
            self.T = tf.compat.v1.assign(self.T, new_transition)
            # self.count = tf.compat.v1.assign_add(self.count, 1)
    ...
    def run_Baum_Welch_EM(self, obs_seq, summary=False, monitor_state_1=False):
        ...
        with tf.Session() as sess:
            ...
            trans0, transition, emission, c = sess.run(
                [self.T0, self.T, self.E, converged]
            )
